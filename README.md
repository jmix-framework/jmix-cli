...
Generated projects come with the
[Jmix Agent Toolkit](https://github.com/jmix-framework/jmix-agent-toolkit)
installed automatically: guidelines files and project-local skills for AI
coding agents (Claude, Codex, OpenCode, Junie). After generation the wizard
offers to open the new project — in IntelliJ IDEA when it is installed,
otherwise in the system file manager — and run it right away; once the
application starts, it opens in the browser. When no compatible
